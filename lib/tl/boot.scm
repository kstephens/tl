...
(define (char<? a b) (tl_b (< (char->integer a) (char->integer b))))
(define (char=? a b) (tl_b (= (char->integer a) (char->integer b))))
...
(define (%string-cmp a b)
  (if (eq? a b) 0
    (let ((cmp (tl_i (memcmp (%string-ptr a) (%string-ptr b) (tl_I (min (string-length a) (string-length b)))))))
      (if (zero? cmp) (sign (- (string-length a) (string-length b))) (if (= cmp 1) 1 -1)))))
...
(define (substring s i n)
  (let ((o (%make-string (%malloc-atomic (+ n 1)) n)))
