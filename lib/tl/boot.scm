  ...
  (let ((fp (fopen (->char* f) (->char* m))))
    (if (eq? fp %NULL) #f
      (%register-finalizer
        ...
        close-port))))
(define (open-input-file f)  (open-file f "r"))
...
(define (close-port p)
  ...
  (set-car! p #f)
  p)
(set! tl_stdin  (%make-port (->FILE* tl_stdin)  '(tl_stdin  "r")))
(set! tl_stdout (%make-port (->FILE* tl_stdout) '(tl_stdout "w")))
(set! tl_stderr (%make-port (->FILE* tl_stderr) '(tl_stderr "w")))
...
(define (read . port)
  (tl_read (if (null? port) tl_stdin (car port))))
...
(%object-writer <closure>
  (lambda (o p op)
    (fprintf (->FILE* p) (tl_S "#<%s @%p ") (tl_type_name (tl_type o)) o)
    (tl_object_write (tl_get o 2) p op)
    (tl_string_display " " p)
