...
(define (prim? exp)
  (or (primitive? exp) (c-func? exp) (prim-ref? exp)))
(define prim-ref-cache '())
(define (prim-ref? exp)
  (if (symbol? exp)
    (let ((x (assq exp prim-ref-cache)))
      (if x (cdr x)
        (let ((slot (tl-lookup-slot exp)))
          (let ((result (and slot (primitive? (cdr slot)) (cdr slot))))
            (set! prim-ref-cache (cons (cons exp result) prim-ref-cache))
            (if result (begin (display "   prim-ref-cache = " tl_stderr)(write prim-ref-cache tl_stderr)(newline tl_stderr)))
            result))))
    #f))

(define (prim->name prim)
  (if (c-func? prim) (symbol->string (c-func->name prim))
    (primitive->name prim)))
